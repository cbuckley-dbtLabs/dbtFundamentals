...
with
    ...
    daily_lifecycle as (
        ...
        select
            ...
            count(
                case when is_active_in_window then customer_id end
            ) as active_customer_count,
            count(
                case when lifetime_orders_to_date > 0 then customer_id end
            ) as customers_with_orders_to_date,
            ...
            sum(delivered_orders_in_window) as delivered_orders_in_window,
            sum(refunded_orders_in_window) as refunded_orders_in_window,
            ...
            round(
                avg(days_since_most_recent_order), 1
            ) as avg_days_since_most_recent_order
...
select
    ...
    refund_amount_in_window / nullif(revenue_in_window, 0) as refund_rate_in_window
