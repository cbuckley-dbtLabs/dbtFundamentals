with
    ...
    order_payments as (
        select order_id, total_amount, gift_card_amount, credit_card_amount
...
select
    ...
    coalesce(
        state_demo_order_enrichment.approved_refund_amount, 0
    ) as approved_refund_amount,
    coalesce(
        state_demo_order_enrichment.pending_refund_amount, 0
    ...
    coalesce(
        state_demo_order_enrichment.distinct_product_count, 0
    ...
    coalesce(
        state_demo_order_enrichment.is_fulfillment_return, 0
    ) as is_fulfillment_return,
